...
WorkTime = {
    ...
    NodeJsServer = '192.168.110.101',  -- Location of deployed NodeJS server
    StationSsid  = 'Since1995',         -- Name of your wifi network
    StationPwd   = '09881654z'        -- Pasword of your wifi network
};
...
function initRC522()
    ...
    mytimer = tmr.create()
    mytimer:register(250, tmr.ALARM_AUTO, function()
        ...
        if isTagNear == true then
            ...
            if table.getn(serialNo) == 5 then
                ...
                http.post('http://192.168.110.101:4000/post-test' ,'Content-Type:application/x-www-form-urlencoded\r\n' ,'UIDresult='..appendHex(serialNo),
                    nil,
                    function(code, data)
                        gpio.write(WorkTime.LED_BLUE, gpio.HIGH)
